...
int encode_image(jpeg_data *data, int (*read_data_func)(uint32_t pos, uint8_t *data, uint32_t len), int (*write_data_func)(uint8_t *data, uint32_t len)) {
	...
	for( cur_mcu_height = 0; cur_mcu_height < mcu_height_max; cur_mcu_height++) {
	  for( cur_mcu_width = 0; cur_mcu_width < mcu_width_max; cur_mcu_width++) {
		  for( cur_height = 0; cur_height < mcu_size; cur_height++) {
			  if((cur_mcu_height*mcu_size + cur_height) >= height) {
				  for(i = 0; i < mcu_size*pixel_size_by_byte; i++) {
					  if(i%2 == 0) {
				  ...
				  for( cur_width = 0; cur_width < mcu_size*pixel_size_by_byte; cur_width++) {
 ...
 * @return JPEG file size. Under 0 is error.
 */
int encode_jpeg(jpeg_data *data, uint8_t *in, uint8_t *out) 
{
    int out_ptr;
    int i,j;
	int mcu_size = data->c_info.mcu_size;
    ...
    out_ptr = 0;
    for(j = 0;j < data->data_len;j++){
        out[out_ptr++] = data->ret_data[j];
    }
                ...
                if((cur_mcu_height*mcu_size + cur_height) >= height) {
                    ...
                    for(cur_width = 0; cur_width < mcu_size*pixel_size_by_byte; cur_width++) {
                        if(cur_mcu_width*mcu_size*pixel_size_by_byte + cur_width >= width*pixel_size_by_byte) {
                            ...
                            mcu[cur_height*mcu_size*pixel_size_by_byte + cur_width] = in[(cur_mcu_height*mcu_size + cur_height)*width*pixel_size_by_byte + (cur_mcu_width*mcu_size*pixel_size_by_byte + cur_width)];
                            if(cur_width%2 == 0) {
                                prePixel_Crb =  mcu[cur_height*mcu_size*pixel_size_by_byte + cur_width];
                            } else {
                                prePixel_y =  mcu[cur_height*mcu_size*pixel_size_by_byte + cur_width];
...
static int split_MCU(jpeg_data *data, const uint8_t *mcu) {
    ...
    for(y = 0; y < 2; y++) {
        for(x = 0; x < 2; x++) {
            for(cur_block_height = 0; cur_block_height < BLOCK_SIZE; cur_block_height++) {
                for(cur_block_width = 0; cur_block_width < BLOCK_SIZE; cur_block_width++) {
