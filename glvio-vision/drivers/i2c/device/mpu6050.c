...
#define ICMREG_LP_MODE_CFG          0x1E
#define ICMREG_ACCEL_WOM_THR        0x1F
...
{
	uint8_t result;
	struct mpu6050_config_s *cfg = (struct mpu6050_config_s *)dev->priv_data;
	hal_i2c_read_from_addr(dev,MPU6050REG_WHO_AM_I,&result,1);
	if(result == MPU6050_WHO_AM_I_VALUE){
	    cfg->id = MPU6050_WHO_AM_I_VALUE;
		printf("[MPU6050]mpu6050 found\n");
		return 0;
	}else if(result == MPU6050A_WHO_AM_I_VALUE){
	    cfg->id = MPU6050A_WHO_AM_I_VALUE;
		printf("[MPU6050]mpu6050a found\n");
		return 0;
	}else{
		printf("[MPU6050]init mpu6050 failed\n");
	}
	return -1;
}
...
static int mpu6050_init_device(struct hal_dev_s *dev)
{
	...
	if(cfg->id == MPU6050_WHO_AM_I_VALUE){
    	reg_val = 0x80;
	...
	}else if(cfg->id == MPU6050A_WHO_AM_I_VALUE){
        reg_val = 0x80;
    	result = hal_i2c_write_to_addr(dev,MPU6050REG_PWR_MGMT_1, &reg_val,1); //enter standby
    	if(result < 0){
    		printf("[%s] set MPU6050REG_PWR_MGMT_1 failed\r\n",__func__);
	    ...
	    reg_val = 0x00;
    	result = hal_i2c_write_to_addr(dev,ICMREG_PWR_MGMT_2, &reg_val,1); //enter standby
    	if(result < 0){
    		printf("[%s] set MPU6050REG_PWR_MGMT_2 failed\r\n",__func__);
    		return -1;
    	}

    	reg_val = 0x10;
    	result = hal_i2c_write_to_addr(dev,MPU6050REG_ACCEL_CONFIG, &reg_val,1); //enter standby
    	if(result < 0){
    		printf("[%s] set MPU6050REG_ACCEL_CONFIG failed\r\n",__func__);
    		return -1;
    	}
    	...
    	reg_val = 0x05;
    	result = hal_i2c_write_to_addr(dev,ICMREG_ACCEL_CONFIG2, &reg_val,1); //enter standby
    	if(result < 0){
    		printf("[%s] set MPU6050REG_ACCEL_CONFIG2 failed\r\n",__func__);
    		return -1;
    	}
    	...
    	reg_val = 0x18;
    	result = hal_i2c_write_to_addr(dev,MPU6050REG_GYRO_CONFIG, &reg_val,1); //enter standby
    	if(result < 0){
    		printf("[%s] set MPU6050REG_GYRO_CONFIG failed\r\n",__func__);
    		return -1;
    	}
