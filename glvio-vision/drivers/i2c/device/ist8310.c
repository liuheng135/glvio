...
int ist8310_init(struct hal_dev_s *dev)
{
	...
	ist8310_delay_us(4000);
	//hal_i2c_read_from_addr(dev,IST8310_WHO_AM_I,&reg_val,1);
    
	reg_val = IST8310_WHO_AM_I;
	msg.buff   = &reg_val;
	msg.length = 1;
	msg.flags  = HAL_I2C_WR;
    hal_i2c_transfer(dev,&msg,1);
    ist8310_delay_us(5000);
	msg.buff   = &reg_val;
	msg.length = 1;
	...
	if(reg_val != IST8310_WHO_AM_I_VALUE){
	    HAL_INFO("[IST8310] wrong id value [0x%x]\r\n",reg_val);
		return -1;
	}
	reg_val = CTRL4_SRPD;
	hal_i2c_write_to_addr(dev,IST8310_CONTROL_REGISTER4, &reg_val,1);  
	ist8310_delay_us(2000);
	reg_val = CTRL1_MODE_SINGLE;
	hal_i2c_write_to_addr(dev,IST8310_CONTROL_REGISTER1, &reg_val,1);  
	reg_val = CTRL3_SAMPLEAVG_0;
	hal_i2c_write_to_addr(dev,IST8310_CONTROL_REGISTER3, &reg_val,1);  
...
{
	ist8310.port	  = 0;
	ist8310.address   = 0x0C;
	ist8310.cfg.flags = HAL_I2C_WR | HAL_I2C_RD;
	ist8310.cfg.speed = 400000;
	ist8310.cfg.width = 8;
